...
const query = require('./pinceau.90146cac.cjs');
require('ohash');
require('scule');
...
function loadVueStyle(query$1, ctx) {
  const { filename } = query$1;
  const file = node_fs.readFileSync(filename, "utf8");
  if (!file) {
    return;
  }
...
async function resolveConfigLayer({
  ...
  extensions.some((_ext) => {
    if (node_fs.existsSync(path$1 + _ext)) {
...
async function importConfigFile({ path, ext }) {
  ...
  const jiti$1 = jiti.createJiti(path, {
    cache: false,
    requireCache: false,
    interopDefault: true
  });
  console.log({
    path,
    imported: await jiti$1.import(path, { default: true })
  });
  return {
    path,
    content,
    config: await jiti$1.import(path, { default: true })
  };
}
...
function usePinceauConfigContext(options, getViteServer, getTransformed, dispatchConfigUpdate) {
  ...
  async function reloadConfig(newOptions = options) {
    ...
    cwd = newOptions?.cwd ?? process__default.cwd();
...
async function prepareOutputDir({
  ...
  if (!node_fs.existsSync(themeDir)) {
...
async function stubOutputs(buildPath, force = false, schema = false) {
  ...
  for (const [file, stubbingFunction] of Object.entries(files)) {
    ...
    if (force && node_fs.existsSync(path$1)) {
