...
function useDebugPerformance(text, debug, logOnStop = true) {
  const isDebug = debug === 2;
  const performanceTimerStart = node_perf_hooks.performance.now();
  ...
  function stop(silent = false) {
    ...
    performanceTimerStop = node_perf_hooks.performance.now();
    if (logOnStop) {
...
function updateDebugContext(newContext) {
  context = {
    ...context,
    ...newContext
  };
...
function transformCssFunction(id, code = "", variants, computedStyles, localTokens, ctx, loc) {
  try {
