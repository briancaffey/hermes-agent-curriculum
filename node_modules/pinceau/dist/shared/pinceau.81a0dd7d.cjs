...
require('defu');
require('acorn');
const query = require('./pinceau.5a583837.cjs');
require('vue/compiler-sfc');
require('recast');
require('recast/parsers/typescript.js');
require('ohash');
require('scule');
const context = require('./pinceau.6f7ae971.cjs');
require('vue');
...
const pinceau = unplugin.createUnplugin((options) => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      const magicString = new MagicString(code, { filename: query$1.filename });
      const result = () => {
        stopPerfTimer2();
      ...
      try {
        if (query.JS_EXTENSIONS.includes(query$1.ext)) {
          return { code: context.transformDtHelper(code, ctx) };
        }
        if (query$1.styles && !query$1.vue || query$1.type === "style") {
          return { code: context.resolveStyleQuery(code, magicString, query$1, ctx, loc).code };
        }
